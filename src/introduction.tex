...
We propose a develop flow to use AXI-Stream IP, with a little modified of UIO driver and little trick setting in device tree file. We also provide functions called \textbf{read()/write()} which send DMA transactions to DMA controller. Whole scenario is very simple and intuitive, and is not large different from normal flow. The efficiency is also nice, 
